...
**Before every push, run `npm run prepush`** (WAP-203). In about 1–2 minutes it runs, failing fast: the migration-history, source-text-test, admin-audit, high-risk tenant-route and tenant-scoping-ratchet guards, the Material Symbols check, `tsc --noEmit`, ESLint on the files you changed versus `origin/master` plus the `npm run lint` guard scripts, and `vitest --changed` for tests related to your changes. It is the same script (`scripts/prepush.mjs`) CI's `static-gates` job runs with `--ci` (whole-repo lint plus knip instead of `vitest --changed`), so a green prepush means the static gates will be green in CI. Measured 2026-09-23: about 35 s for a two-file lib/component change with a warm `tsc` cache, about 100 s cold. A change to `package.json` or the Vitest/Vite config makes `vitest --changed` rerun the whole suite (Vitest's `forceRerunTriggers`), which takes about 5 minutes. Run `git fetch origin master` first so the base is current; `--base <ref>` compares against another ref and `--list` prints the plan. The tenant pin is read from `.github/workflows/ci-gate.yml`; never raise it. To run it automatically on every `git push`, opt in once per clone or worktree with `git config core.hooksPath .githooks` (skip a single push with `git push --no-verify`). No Husky or other dependency is involved. `npm run prepush` does not replace the full unit, Vitest and build lanes; CI still runs all of them.

```bash
npm run prepush       # Fast pre-push gate: guards, typecheck, changed-file lint, related Vitest
