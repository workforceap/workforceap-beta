...
- [Authenticated Portal Smoke](../../.github/workflows/authenticated-portal-smoke.yml#L24) runs nightly on trusted `master` against an isolated Preview, and also supports manual dispatch including a non-staff production canary. It checks the deployed version and database scope before signing in. Target and role credentials are workflow secrets; inspect its mode before invocation. [Public E2E](../../.github/workflows/nightly-e2e.yml) runs credential-free read-only production specs nightly. Scheduled failures create one issue per suite; see [Nightly heavy suites](../SLO-AND-STATUS.md#nightly-heavy-suites-wap-204).
