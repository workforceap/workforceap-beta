...
[Required CI](../../.github/workflows/ci-gate.yml#L3) runs on PRs and pushes to `master`, plus manual dispatch; concurrent runs for the same ref are canceled. The `ci` job first verifies migration timestamp history, then performs root frozen install, marketing `npm ci`/build/copy, Prisma generation, typecheck, font verification, Node tests, lint, Vitest, tenant-route verification, the source-text test guard, Next build and PDF deployment verification. Knip is report-only. A second `database-contract` job stands up a PostgreSQL 16 service and runs `scripts/run-db-contract-tests.mjs` (schema push, migration proofs, the member-message RLS proof and the real-database role tests), uploading the RLS proof artifact. CI uses declared dummy configuration and does not establish real provider access or production migration readiness; the pushed schema is not a replay of `prisma/migrations`.
