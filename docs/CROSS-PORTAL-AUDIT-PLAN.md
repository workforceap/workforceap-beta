...
4. Inspect **`test-results/portal-audit-results.json`**. The artifact includes an explicit `evidenceScope` (`complete_five_role_matrix`, `production_nonstaff_canary`, or local subset), route results, actual authenticated-identity distinctness, and every applicable allowed/denied access-matrix probe. A fresh failed/incomplete artifact is written before browser work starts.
   Safe partner, employer, and counselor navigation actions wait for a new visible destination heading after the URL changes, then run the ordinary destination health and read-only checks.
   ...
   On a manual isolated Preview run, `capture_hydration_trace` may be enabled to log allowlisted route templates and root structure only when a rendered or redirect-only probe reports React #418. The trace also records only the fixed React #418 decoder category (`HTML` or `text`) when its raw in-memory error URL matches the installed React format; the URL, query, and other error arguments are never logged. This category does not identify a component or root cause. The trace is diagnostic and does not change the row verdict.
...
Both the target deployment and the workflow runner require the same secret `PORTAL_AUDIT_READ_ONLY_TOKEN` (minimum 32 characters). The token must be stored only in the deployment/workflow secret stores; it must never be committed, logged, or copied into the result artifact. Each audit context receives a host-only, HttpOnly, SameSite=Strict browser cookie (Secure for HTTPS); captured role storage state has that cookie removed before reuse. A missing or mismatched token blocks the audit.
...
- Authentication is the only permitted write: the login route may update its dedicated test account's login telemetry. The browser sends the external capability as a host-only cookie for the validated target. Middleware removes that cookie from the forwarded request, discards caller-supplied audit headers, compares the cookie in constant time, and mints the internal audit header only after authentication. A naked public header cannot enable audit behavior. Browser cookie scoping keeps the capability off redirects to other hosts; cookies are scoped by host rather than port.
...
| Date | Change |
|------|--------|
| 2026-09-25 | Require a fresh destination heading for the partner referred-members and counselor students navigation actions, matching the employer jobs action. |
