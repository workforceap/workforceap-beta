...
- **Member nav (live):** warm left command rail on desktop (`MEMBER_PORTAL_NAV_ITEMS` via `WorkspaceShell`; Home · My program · Job board · AI Career Tools · Messages stay visible, with My progress and Skill missions under Training & progress — WAP-189) and sticky horizontal top tabs on mobile (`MemberPortalTopNav`: Dashboard · Program · Jobs · Progress · Messages · Toolkit · Profile). Full IA stays in the rail/drawer — do not CSS-hide the member sidebar alone. Staff portals keep the dense left rail; see `docs/PORTAL_NAV_SPEC.md`.
