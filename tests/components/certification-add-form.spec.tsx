 ...
 * certificate keeps its review state), and paint on kit tokens.
 */
...
import CertificationAddForm, { certificationAddedNotice } from '@/components/portal/CertificationAddForm';
...
describe('CertificationAddForm', () => {
  ...
  it('saves a self-report as pending, confirms it in place and refreshes the server list', async () => {
    ...
    expect(await screen.findByRole('status')).toHaveTextContent('OSHA 10 added. It shows as pending until our staff check it.');
    ...
    expect(screen.getByRole('button', { name: 'Add another certificate' })).toBeInTheDocument();
  ...
  it('a re-added certificate that staff already verified is not called pending', async () => {
    ...
    expect(await screen.findByRole('status')).toHaveTextContent('CompTIA A+ is already on your list and verified.');
    expect(screen.getByRole('status')).not.toHaveTextContent(/pending/i);
  });
