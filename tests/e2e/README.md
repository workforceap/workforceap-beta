...
`.github/workflows/nightly-e2e.yml` runs nightly and supports manual dispatch. It runs the credential-free, read-only specs (`tests/e2e/smoke/**` and `partner-signup-viewports.spec.ts`) against production. A failed scheduled run opens or refreshes one `Nightly: Public E2E failing` issue (WAP-204/WAP-216; see `docs/SLO-AND-STATUS.md`, "Nightly heavy suites"). Only add a spec there if it signs in to nothing and creates no data.
